...
# wkhtmltopdf.sh 의 PNG 버전. 같은 wkhtmltopdf 서비스 컨테이너에 동봉된 wkhtmltoimage 바이너리를 사용한다.
COMPOSE_FILE=${COMPOSE_FILE:-docker/docker-compose.yml}
SERVICE=${WKHTMLTOPDF_SERVICE:-wkhtmltopdf}
CONTAINER_DIR=${WKHTMLTOPDF_CONTAINER_DIR:-/tmp/resume-pdf}

if [ "$#" -lt 2 ]; then
  echo "Usage: ${0} [wkhtmltoimage options] <input-html> <output-png>" >&2
  exit 1
fi
...
# abs_path 는 부모 디렉터리로 cd 하므로, 출력 디렉터리를 먼저 만들어 두어야 새 경로에서도 실패하지 않는다.
...
container_output="$CONTAINER_DIR/${unique}.png"
...
compose exec -T "$SERVICE" wkhtmltoimage "${options[@]}" "$container_input" "$container_output"
